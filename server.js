...
export function createGameServer(){
 ...
 io.on('connection',s=>{
  ...
  s.on('input',(v={})=>{const r=rooms.get(s.data.code),p=r?.players[s.id];if(!p||!v||typeof v!=='object'||!['prep','play'].includes(r.phase))return;
   p.input={x:Number.isFinite(v.x)?Math.max(-1,Math.min(1,v.x)):0,z:Number.isFinite(v.z)?Math.max(-1,Math.min(1,v.z)):0,fire:!!v.fire,jump:!!v.jump,spin:Number.isFinite(v.spin)?Math.max(-1,Math.min(1,v.spin)):0};
