...
setInterval(()=>{if(!active||!socket.connected)return;const blocked=!entered||!!document.querySelector('dialog[open]')||me()?.status!=='alive',f=blocked?0:(keys.w?1:0)-(keys.s?1:0),side=blocked?0:(keys.d?1:0)-(keys.a?1:0);
 ...
 socket.emit('input',{x:-Math.sin(yaw)*f+Math.cos(yaw)*side,z:-Math.cos(yaw)*f-Math.sin(yaw)*side,yaw,pitch,fire:!blocked&&!hiding&&(fire||!!keys[' ']),jump:!blocked&&hiding&&!!keys[' '],spin:blocked?0:(keys.x?1:0)-(keys.z?1:0),lift:blocked?0:((keys.pageup?1:0)-(keys.pagedown?1:0))||(performance.now()-wheelAt<140?wheelLift:0)});},50);
