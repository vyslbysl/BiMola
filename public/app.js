...
const defaults={mapId:'loft',teamSize:3,botMode:'fill',hunterBots:3,hiderBots:2,hideSeconds:20,roundSeconds:180,teamSelection:'choose',swapTeams:true,objectCount:51,decor:1,idleReveal:30,revealHits:3,escapeBoost:1.1};
...
function openPlay(newMode){mode=newMode;$('quick-map-field').classList.toggle('hidden',mode!=='quick');$('quick-map').value=draft.mapId||'loft';$('play-error').textContent='';$('join-code-field').classList.toggle('hidden',mode!=='join');$('play-description').textContent=mode==='quick'?"3'e 3 hızlı oyun. Eksik yerleri botlar tamamlar.":mode==='create'?`${draft.teamSize}'e ${draft.teamSize} oda. Takımını seç, arkadaşlarını çağır.`:'Arkadaşının kodunu yaz ve takımını seç.';$('play-title').textContent=mode==='join'?'Arkadaşlarının yanına gel.':'Bugün kim olacaksın?';$('confirm-play').textContent=mode==='quick'?'Hızlı oyuna başla →':mode==='create'?'Odayı oluştur →':'Odaya katıl →';showDialog('play-dialog');}
$('quick').onclick=()=>openPlay('quick');$('join-open').onclick=()=>openPlay('join');
for(const map of MAP_CHOICES)for(const id of ['map-id','quick-map']){const o=document.createElement('option');o.value=map.id;o.textContent=map.name;$(id).append(o);}
function renderMapPreview(){const map=MAP_CHOICES.find(m=>m.id===$('map-id').value)||MAP_CHOICES[0];$('map-preview-image').src=map.preview;$('map-preview-image').alt=map.name+' görsel referansı';$('map-preview-name').textContent=map.name;$('map-preview-note').textContent=map.subtitle;}
for(let i=1;i<=12;i++){const o=document.createElement('option');o.value=i;o.textContent=`${i} kişi · ${i}'e ${i}`;$('team-size').append(o);}
function showSettings(settings,editing){editSettings=editing;draft={...defaults,...settings};$('map-id').value=draft.mapId;renderMapPreview();$('team-size').value=draft.teamSize;$('bot-mode').value=draft.botMode;$('hider-bots').value=draft.hiderBots;$('hunter-bots').value=draft.hunterBots;$('hide-seconds').value=draft.hideSeconds;$('round-seconds').value=draft.roundSeconds;$('object-count').value=draft.objectCount;$('decor').value=draft.decor;$('idle-reveal').value=draft.idleReveal;$('reveal-hits').value=draft.revealHits;$('escape-boost').value=draft.escapeBoost;$('team-selection').value=draft.teamSelection;$('settings-title').textContent=editing?'Odayı sen ayarla.':'Kendi odanı kur.';$('save-settings').textContent=editing?'Ayarları kaydet →':'Devam et →';$('settings-error').textContent='';updateSettings();showDialog('settings-dialog');}
function readSettings(){return{mapId:$('map-id').value,teamSize:Number($('team-size').value),botMode:$('bot-mode').value,hiderBots:Number($('hider-bots').value),hunterBots:Number($('hunter-bots').value),hideSeconds:Number($('hide-seconds').value),roundSeconds:Number($('round-seconds').value),objectCount:Number($('object-count').value),decor:Number($('decor').value),idleReveal:Number($('idle-reveal').value),revealHits:Number($('reveal-hits').value),escapeBoost:Number($('escape-boost').value),teamSelection:$('team-selection').value,swapTeams:true};}
function updateSettings(){renderMapPreview();const s=readSettings(),custom=s.botMode==='custom';$('hider-bot-field').classList.toggle('hidden',!custom);$('hunter-bot-field').classList.toggle('hidden',!custom);$('hider-bots').max=$('hunter-bots').max=s.teamSize;$('settings-summary').textContent=`${s.teamSize*2} kişilik oda · ${s.hideSeconds} sn saklanma · ${s.roundSeconds/60} dk tur · ${s.objectCount} ek eşya · süs %${Math.round(s.decor*100)} · ${s.idleReveal?`${s.idleReveal} sn kıpırdamayanın izi çıkar`:'iz kapalı'} · oyuncuyu açığa çıkarmak için ${s.revealHits} isabet · ıslanan ${s.escapeBoost}× hızla kaçar. ${s.botMode==='fill'?'Eksik yerler botlarla dolar.':s.botMode==='off'?'Tüm yerler gerçek oyuncular için.':'Bot sayılarını lobide de değiştirebilirsin.'}`;}
for(const id of ['map-id','team-size','bot-mode','hider-bots','hunter-bots','hide-seconds','round-seconds','object-count','decor','idle-reveal','reveal-hits','escape-boost','team-selection'])$(id).onchange=updateSettings;
...
$('confirm-play').onclick=async()=>{if(!socket.connected){$('play-error').textContent='Sunucuya bağlanılması bekleniyor.';return;}const code=mode==='join'?$('code').value.replace(/\D/g,''):'';if(mode==='join'&&code.length!==4){$('play-error').textContent='4 haneli oda kodunu yaz.';return;}$('confirm-play').disabled=true;try{localStorage.setItem('mola-name',$('name').value);}catch{}
 request('join',{name:$('name').value,role:selectedRole,practice:mode==='quick',code,settings:mode==='quick'?{...defaults,mapId:$('quick-map').value,botMode:'fill'}:draft},r=>{$('confirm-play').disabled=false;if(r.error){$('play-error').textContent=r.error;return;}myId=r.id;lastRound=0;lastPhase='';teamSignature='';pickerHush='';pickerAuto=false;entered=false;closeDialogs();$('error').textContent='';});};
...
function renderLobby(s){$('copy').textContent=s.code;const host=s.host===myId;$('settings-open').disabled=!host;document.querySelectorAll('.bot-controls').forEach(x=>x.classList.toggle('hidden',!host));$('lobby-settings').textContent=`${MAP_CHOICES.find(m=>m.id===s.settings.mapId)?.name||'Güneşli Ev'} · ${s.settings.teamSize}'e ${s.settings.teamSize} · ${s.settings.roundSeconds/60} dk tur · ${s.settings.revealHits} isabet · ${s.settings.escapeBoost}× kaçış`;
