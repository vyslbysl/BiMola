...
socket.on('state',s=>{if(!myId)return;state=s;const p=me();if(!p)return;if(s.round!==lastRound&&['brief','prep'].includes(s.phase)){lastRound=s.round;spectatorMode='free';spectatorTarget=null;entered=false;yaw=p.yaw||0;pitch=0;stopInput();pickerHush='';pickerAuto=false;pickerSignature='';$('picker').classList.add('hidden');}
 ...
 $('controls-hider').classList.toggle('hidden',hunting);$('controls-hunter').classList.toggle('hidden',!hunting);$('touch-fire').classList.toggle('hidden',!hunting);$('touch-jump').classList.toggle('hidden',hunting);$('touch-crouch').classList.toggle('hidden',!hunting);$('decoy-count').textContent=p.decoys??0;$('decoy-button').disabled=!p.propId||!p.decoys||p.status!=='alive'||!['prep','play'].includes(s.phase);$('change-count').textContent=p.changes;$('shuffle-button').disabled=!p.propId||p.changes<=0||p.status!=='alive';$('pick-button').disabled=!!p.propId||p.status!=='alive';$('lock-button').disabled=!p.propId||p.status!=='alive';$('lock-button').querySelector('span').textContent=p.locked?'Hareket et':'Sabitle';$('lock-button').classList.toggle('selected',p.locked);
