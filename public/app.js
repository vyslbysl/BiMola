...
const defaults={teamSize:3,botMode:'fill',hunterBots:3,hiderBots:2,hideSeconds:20,roundSeconds:180,teamSelection:'choose',swapTeams:true,objectCount:51};
let state=null,myId=null,active=false,entered=false,keys={},yaw=0,pitch=0,fire=false,dragging=false,dragDistance=0,mode='quick',selectedRole='hider',draft={...defaults},editSettings=false,lastRound=0,lastPhase='',teamSignature='',pickerSignature='',lastShotId='',toastTimer,hitTimer,lastAutoPicker=false;
...
function grabMouse(){
 ...
 if(document.querySelector('dialog[open]')||!$('picker').classList.contains('hidden'))return;
 lockWanted=true;
...
document.addEventListener('pointerlockchange',()=>{
 ...
 lockWanted=false;stopInput();
 if(!active||!entered||document.querySelector('dialog[open]')||!$('picker').classList.contains('hidden'))return;
 showDialog('pause-dialog');
});
function showDialog(id){lockWanted=false;unlock();if(!$(id).open)$(id).showModal();}
function closePicker(regrab=true){$('picker').classList.add('hidden');if(regrab)grabMouse();}
function closeDialogs(){document.querySelectorAll('dialog[open]').forEach(d=>d.close());}
function setScreen(id){for(const el of ['home','lobby','hud','result'])$(el).classList.toggle('hidden',el!==id);active=id==='hud';document.body.classList.toggle('playing',active);if(!active){document.body.classList.remove('hunter-wait');entered=false;unlock();$('picker').classList.add('hidden');}}
...
function showSettings(settings,editing){editSettings=editing;draft={...defaults,...settings};$('team-size').value=draft.teamSize;$('bot-mode').value=draft.botMode;$('hider-bots').value=draft.hiderBots;$('hunter-bots').value=draft.hunterBots;$('hide-seconds').value=draft.hideSeconds;$('round-seconds').value=draft.roundSeconds;$('object-count').value=draft.objectCount;$('team-selection').value=draft.teamSelection;$('settings-title').textContent=editing?'Odayı sen ayarla.':'Kendi odanı kur.';$('save-settings').textContent=editing?'Ayarları kaydet →':'Devam et →';$('settings-error').textContent='';updateSettings();showDialog('settings-dialog');}
function readSettings(){return{teamSize:Number($('team-size').value),botMode:$('bot-mode').value,hiderBots:Number($('hider-bots').value),hunterBots:Number($('hunter-bots').value),hideSeconds:Number($('hide-seconds').value),roundSeconds:Number($('round-seconds').value),objectCount:Number($('object-count').value),teamSelection:$('team-selection').value,swapTeams:true};}
function updateSettings(){const s=readSettings(),custom=s.botMode==='custom';$('hider-bot-field').classList.toggle('hidden',!custom);$('hunter-bot-field').classList.toggle('hidden',!custom);$('hider-bots').max=$('hunter-bots').max=s.teamSize;$('settings-summary').textContent=`${s.teamSize*2} kişilik oda · ${s.hideSeconds} sn saklanma · ${s.roundSeconds/60} dk tur · ${s.objectCount} nesne. ${s.botMode==='fill'?'Eksik yerler botlarla dolar.':s.botMode==='off'?'Tüm yerler gerçek oyuncular için.':'Bot sayılarını lobide de değiştirebilirsin.'}`;}
for(const id of ['team-size','bot-mode','hider-bots','hunter-bots','hide-seconds','round-seconds','team-selection'])$(id).onchange=updateSettings;
...
$('confirm-play').onclick=async()=>{if(!socket.connected){$('play-error').textContent='Sunucuya bağlanılması bekleniyor.';return;}const code=mode==='join'?$('code').value.trim().toUpperCase():'';if(mode==='join'&&code.length!==6){$('play-error').textContent='6 karakterli oda kodunu yaz.';return;}$('confirm-play').disabled=true;try{await audio.set(true);soundUI();}catch{}try{localStorage.setItem('mola-name',$('name').value);}catch{}
 request('join',{name:$('name').value,role:selectedRole,practice:mode==='quick',code,settings:mode==='quick'?{...defaults,botMode:'fill'}:draft},r=>{$('confirm-play').disabled=false;if(r.error){$('play-error').textContent=r.error;return;}myId=r.id;lastRound=0;lastPhase='';teamSignature='';lastAutoPicker=false;entered=false;closeDialogs();$('error').textContent='';});};
...
function renderLobby(s){$('copy').textContent=s.code;const host=s.host===myId;$('settings-open').disabled=!host;document.querySelectorAll('.bot-controls').forEach(x=>x.classList.toggle('hidden',!host));$('lobby-settings').textContent=`${s.settings.teamSize}'e ${s.settings.teamSize} · ${s.settings.roundSeconds/60} dk tur`;
 ...
 const full=['hider','hunter'].every(t=>s.players.filter(p=>p.team===t).length===s.settings.teamSize);$('start').disabled=!host||!full;$('waiting').textContent=!host?'Oda kurucusu turu başlatacak.':full?'İki takım da hazır.':"Başlamak için iki takımı doldur. İstersen bot ekle.";}
function perform(kind,objectId){if(!active||!entered||me()?.status!=='alive')return;request('action',{kind,objectId},r=>{if(!r.ok){if(r.error)toast(r.error);return;}if(kind==='possess'){closePicker();audio.effect('transform',.13);toast(`${propTypes[r.type]?.name||'Nesne'} oldun. Konumunu ayarla; F ile sabitle.`);}if(kind==='shuffle'){audio.effect('transform',.13);toast(`${propTypes[r.type]?.name} oldun · ${r.changes} değişim hakkın kaldı.`);}if(kind==='lock')toast(r.locked?'Konumun sabitlendi. F ile tekrar hareket edebilirsin.':'Artık hareket edebilirsin.');});}
function showPicker(){const p=me();if(!p||p.role!=='hider'||!entered)return;if(p.propId){toast('İlk nesneni seçtin. Q ile rastgele değiştirebilirsin.');return;}lockWanted=false;unlock();$('picker').classList.remove('hidden');renderPicker();}
$('pick-button').onclick=showPicker;$('picker-close').onclick=closePicker;$('shuffle-button').onclick=()=>perform('shuffle');$('lock-button').onclick=()=>perform('lock');$('reload-button').onclick=()=>perform('reload');
function renderPicker(){if($('picker').classList.contains('hidden'))return;const list=state?.nearby||[],signature=list.map(o=>o.id+o.type+Math.round(o.distance)+Math.round(o.y*3)).join();if(signature===pickerSignature)return;pickerSignature=signature;if(!list.length){const p=document.createElement('p');p.textContent='Yakında seçilebilecek eşya yok. WASD ile bir saksıya, tabureye veya sepete yaklaş.';$('nearby-props').replaceChildren(p);return;}$('nearby-props').replaceChildren(...list.slice(0,8).map(o=>{const b=document.createElement('button'),art=document.createElement('b'),label=document.createElement('span'),distance=document.createElement('small');
 const shot=world.preview?.(o.type);
 if(shot){const img=document.createElement('img');img.src=shot;img.alt=propTypes[o.type].name;img.style.cssText='width:62px;height:62px;object-fit:contain;display:block';art.append(img);}
 else art.textContent=propTypes[o.type].icon;
 label.textContent=propTypes[o.type].name;distance.textContent=Math.round(o.distance*10)/10+' m'+(o.y>.3?' · yüksekte':'');b.append(art,label,distance);b.onclick=()=>perform('possess',o.id);return b;}));}
function enter(){entered=true;if(state?.phase==='brief')socket.emit('ready');$('brief-card').classList.add('hidden');stopInput();
 if(me()?.role==='hider'&&!me()?.propId){pickerSignature='';showPicker();}else grabMouse();}
...
socket.on('hit',r=>{$('hit-feedback').textContent=r.found?'✦ BULDUN!':`💧 %${Math.round(r.wet)}${r.real?' · Gerçek eşya':''}`;clearTimeout(hitTimer);hitTimer=setTimeout(()=>$('hit-feedback').textContent='',900);if(r.found)audio.effect('found',.15);});
...
socket.on('state',s=>{if(!myId)return;state=s;const p=me();if(!p)return;if(s.round!==lastRound&&['brief','prep'].includes(s.phase)){lastRound=s.round;entered=false;yaw=p.yaw||0;pitch=0;stopInput();lastAutoPicker=false;pickerSignature='';$('picker').classList.add('hidden');}
 if(s.phase!==lastPhase){setScreen(s.phase==='lobby'?'lobby':s.phase==='end'?'result':'hud');lastPhase=s.phase;if(s.phase==='end'){closeDialogs();renderResult(s);}}
 ...
 $('brief-card').classList.toggle('hidden',entered&&!(hunting&&s.phase==='prep')||p.status!=='alive');$('brief-label').textContent=hunting?'SU TABANCASI SENDE':'SAKLANAN SENSİN';$('brief-title').innerHTML=hunting&&s.phase==='prep'?'Gözlerini kapat.<br>Arkadaşların saklanıyor.':hunting?'Şüpheli nesneleri<br>bir güzel ıslat.':'Bir nesne ol.<br>Odaya karış.';$('brief-text').textContent=hunting?'Gerçek eşyaya bir atış yeter, oyuncu için %100 gerekir. Sol tıkla su sık, R ile depoyu doldur.':'Eşyaların yanına git. E ile listeyi açıp ilk nesneni seç. Boşluk ile zıplayıp tezgaha çıkabilir, Z/X ile yönünü ayarlayıp F ile sabitleyebilirsin.';$('brief-rule').textContent=hunting?'Depo: 25 atış. İlk atışta gerçek eşyayı anlarsın; oyuncuysa %100 için 10 isabet gerekir.':'İlk seçim + 3 rastgele değişim. Q ile değiş; su miktarın korunur. Bir kez ıslanınca avcının 2 katı hızla kaçabilirsin.';$('enter').classList.toggle('hidden',entered&&hunting&&s.phase==='prep');$('look-help').textContent=coarse?'Yön tuşlarıyla yürü · Ekranı sürükleyerek bak':hunting?'WASD hareket · Fare bakış · Sol tık / Boşluk su':'WASD hareket · Boşluk zıpla · Z/X çevir';
 ...
 if(p.status==='found'){fire=false;$('picker').classList.add('hidden');}$('found-card').classList.toggle('hidden',p.status!=='found');$('notification').textContent=s.events?.at(-1)?.text||'';
 $('team-status').replaceChildren(...s.players.filter(q=>q.team===p.team).map(q=>{const d=document.createElement('div');d.textContent=(q.status==='found'?'× ':'· ')+q.name+(q.id===myId?' · sen':'');d.className=q.status==='found'?'found':'';return d;}));renderPicker();audio.update(s,myId,yaw);
...
window.addEventListener('keydown',e=>{if(e.target.matches('input,select'))return;const key=aliases[e.key.toLowerCase()]||e.key.toLowerCase();if(key==='escape'&&active&&!document.querySelector('dialog[open]')){e.preventDefault();if(!$('picker').classList.contains('hidden'))closePicker(false);else releaseMouse(true);return;}if(!active||!entered||document.querySelector('dialog[open]')||me()?.status!=='alive')return;if(['w','a','s','d','e','q','f','r','z','x',' '].includes(key)){e.preventDefault();keys[key]=true;if(!e.repeat){if(key==='e')showPicker();if(key==='q')perform('shuffle');if(key==='f')perform('lock');if(key==='r')perform('reload');}}});
...
setInterval(()=>{if(!active||!socket.connected)return;const blocked=!entered||!!document.querySelector('dialog[open]')||me()?.status!=='alive',f=blocked?0:(keys.w?1:0)-(keys.s?1:0),side=blocked?0:(keys.d?1:0)-(keys.a?1:0);
 ...
 socket.emit('input',{x:-Math.sin(yaw)*f+Math.cos(yaw)*side,z:-Math.cos(yaw)*f-Math.sin(yaw)*side,yaw,pitch,fire:!blocked&&!hiding&&(fire||!!keys[' ']),jump:!blocked&&hiding&&!!keys[' '],spin:blocked?0:(keys.x?1:0)-(keys.z?1:0)});},50);
let aimAt=0;world.renderer.setAnimationLoop(t=>{world.render(state,myId,yaw,pitch,active,entered,t);if(t>aimAt&&active&&entered&&me()?.role==='hider'&&!me()?.propId&&$('picker').classList.contains('hidden')){aimAt=t+150;const id=world.pickObject(4.5);const o=state.objects.find(o=>o.id===id);$('aim-label').textContent=o?`E · ${propTypes[o.type].name} ve yakındaki nesneler`:'E · Yakındaki nesneleri seç';}else if(!active||me()?.propId||me()?.role==='hunter')$('aim-label').textContent='';});
const invite=new URLSearchParams(location.search).get('room');if(invite){$('code').value=invite.slice(0,6);openPlay('join');}
