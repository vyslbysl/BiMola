...
function updateSettings(){const s=readSettings(),custom=s.botMode==='custom';$('hider-bot-field').classList.toggle('hidden',!custom);$('hunter-bot-field').classList.toggle('hidden',!custom);$('hider-bots').max=$('hunter-bots').max=s.teamSize;$('settings-summary').textContent=`${s.teamSize*2} kişilik oda · ${s.hideSeconds} sn saklanma · ${s.roundSeconds/60} dk tur · ${s.objectCount} ek eşya. ${s.botMode==='fill'?'Eksik yerler botlarla dolar.':s.botMode==='off'?'Tüm yerler gerçek oyuncular için.':'Bot sayılarını lobide de değiştirebilirsin.'}`;}
for(const id of ['team-size','bot-mode','hider-bots','hunter-bots','hide-seconds','round-seconds','object-count','team-selection'])$(id).onchange=updateSettings;
...
$('confirm-play').onclick=async()=>{if(!socket.connected){$('play-error').textContent='Sunucuya bağlanılması bekleniyor.';return;}const code=mode==='join'?$('code').value.trim().toUpperCase():'';if(mode==='join'&&code.length!==6){$('play-error').textContent='6 karakterli oda kodunu yaz.';return;}$('confirm-play').disabled=true;try{localStorage.setItem('mola-name',$('name').value);}catch{}
...
function showPicker(){$('prop-search').value='';pickerSignature='';const p=me();if(!p||p.role!=='hider'||!entered)return;if(p.propId){toast('İlk nesneni seçtin. Q ile rastgele değiştirebilirsin.');return;}$('picker').classList.remove('hidden');renderPicker();}
...
function quickPossess(){
 ...
 if(aimed&&list.some(o=>o.id===aimed)){if(!$('picker').classList.contains('hidden'))closePicker();perform('possess',aimed);return;}
 if($('picker').classList.contains('hidden'))showPicker();else closePicker();
}
$('decoy-button').onclick=()=>perform('decoy');$('prop-search').oninput=renderPicker;$('pick-button').onclick=showPicker;$('picker-close').onclick=closePicker;$('shuffle-button').onclick=()=>perform('shuffle');$('lock-button').onclick=()=>perform('lock');$('reload-button').onclick=()=>perform('reload');
...
function enter(){entered=true;if(state?.phase==='brief')socket.emit('ready');$('brief-card').classList.add('hidden');stopInput();grabMouse();
 if(me()?.role==='hider'&&!me()?.propId){pickerSignature='';toast('Bir eşyaya bak ve E ile o eşya ol. Hiçbir şeye bakmıyorsan E listeyi açar.');}}
...
socket.on('state',s=>{if(!myId)return;state=s;const p=me();if(!p)return;if(s.round!==lastRound&&['brief','prep'].includes(s.phase)){lastRound=s.round;entered=false;yaw=p.yaw||0;pitch=0;stopInput();lastAutoPicker=false;pickerSignature='';$('picker').classList.add('hidden');}
 ...
 $('controls-hider').classList.toggle('hidden',hunting);$('controls-hunter').classList.toggle('hidden',!hunting);$('touch-fire').classList.toggle('hidden',!hunting);$('touch-jump').classList.toggle('hidden',hunting);$('decoy-count').textContent=p.decoys??0;$('decoy-button').disabled=!p.propId||!p.decoys||p.status!=='alive'||s.phase!=='play';$('change-count').textContent=p.changes;$('shuffle-button').disabled=!p.propId||p.changes<=0||p.status!=='alive';$('pick-button').disabled=!!p.propId||p.status!=='alive';$('lock-button').disabled=!p.propId||p.status!=='alive';$('lock-button').querySelector('span').textContent=p.locked?'Hareket et':'Sabitle';$('lock-button').classList.toggle('selected',p.locked);
 ...
 $('brief-card').classList.toggle('hidden',entered&&!(hunting&&s.phase==='prep')||p.status!=='alive');$('brief-label').textContent=hunting?'SU TABANCASI SENDE':'SAKLANAN SENSİN';$('brief-title').innerHTML=hunting&&s.phase==='prep'?'Gözlerini kapat.<br>Arkadaşların saklanıyor.':hunting?'Şüpheli nesneleri<br>bir güzel ıslat.':'Bir nesne ol.<br>Odaya karış.';$('brief-text').textContent=hunting?'Gerçek eşyaya bir atış yeter, oyuncu için %100 gerekir. Sol tıkla su sık, R ile depoyu doldur.':'Masadan tabağa, sandalyeden musluğa: gördüğün eşyalara yaklaş. E ile ara ve ilk nesneni seç. Boşluk ile zıplayıp tezgaha çıkabilir, Z/X ile yönünü ayarlayıp F ile sabitleyebilirsin.';$('brief-rule').textContent=hunting?'Depo: 25 atış. İlk atışta gerçek eşyayı anlarsın; oyuncuysa %100 için 10 isabet gerekir.':'Q: 3 rastgele değişim. C: 3 kalıcı sahte kopya bırak. Her biri tek isabette dağılır. İlk isabette iki kat hızla kaçabilirsin.';$('enter').classList.toggle('hidden',entered&&hunting&&s.phase==='prep');$('look-help').textContent=coarse?'Yön tuşlarıyla yürü · Ekranı sürükleyerek bak':hunting?'WASD hareket · Fare bakış · Sol tık / Boşluk su':'WASD hareket · Boşluk zıpla · Z/X çevir';
...
window.addEventListener('keydown',e=>{if(e.target.matches('input,select'))return;const key=aliases[e.key.toLowerCase()]||e.key.toLowerCase();if(key==='escape'&&active&&!document.querySelector('dialog[open]')){e.preventDefault();if(!$('picker').classList.contains('hidden'))closePicker(false);else releaseMouse(true);return;}if(!active||!entered||document.querySelector('dialog[open]')||me()?.status!=='alive')return;
 ...
 if(['w','a','s','d','e','q','f','r','c','z','x',' '].includes(key)){e.preventDefault();keys[key]=true;if(!e.repeat){if(key==='e')quickPossess();if(key==='q')perform('shuffle');if(key==='c')perform('decoy');if(key==='f')perform('lock');if(key==='r')perform('reload');}}});
