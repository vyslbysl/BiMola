...
const defaults={mapId:'loft',mapRotate:true,teamSize:3,botMode:'fill',hunterBots:3,hiderBots:2,hideSeconds:20,roundSeconds:180,teamSelection:'choose',swapTeams:true,objectCount:10,decor:.25,idleReveal:30,revealHits:3,escapeBoost:1.1};
...
function updateSettings(){const s=readSettings(),custom=s.botMode==='custom';$('hider-bot-field').classList.toggle('hidden',!custom);$('hunter-bot-field').classList.toggle('hidden',!custom);$('hider-bots').max=$('hunter-bots').max=s.teamSize;$('settings-summary').textContent=`${mapName(s.mapId)}${s.mapRotate?' (her turda değişir)':''} · ${s.teamSize*2} kişilik oda · ${s.hideSeconds} sn saklanma · ${s.roundSeconds/60} dk tur · ${s.objectCount} ek eşya · süs %${Math.round(s.decor*100)} · ${s.idleReveal?`${s.idleReveal} sn kıpırdamayanın izi çıkar`:'iz kapalı'} · oyuncuyu açığa çıkarmak için ${s.revealHits} isabet · ıslanan ${s.escapeBoost}× hızla kaçar. ${s.botMode==='fill'?'Eksik yerler botlarla dolar.':s.botMode==='off'?'Tüm yerler gerçek oyuncular için.':'Bot sayılarını lobide de değiştirebilirsin.'}`;}
for(const id of ['map-rotate','team-size','bot-mode','hider-bots','hunter-bots','hide-seconds','round-seconds','object-count','decor','idle-reveal','reveal-hits','escape-boost','team-selection'])$(id).onchange=updateSettings;
